...
function Cart() {
  ...
  useEffect(() => {
    const fetchShippingCharge = async () => {
      try {
        ...
        if (docSnap.exists()) {
          setShippingCharge(docSnap.data().amount || shippingCharge); 
        }
      } catch (error) {
  ...
  const shipping = totalAmount > 350 ? 'Free Delivery' : shippingCharge;
  const grandTotal = shipping === 'Free Delivery' ? totalAmount : totalAmount + shipping;
          ...
          <div className="rounded-lg md:w-2/3">
            {cartItems.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className="mb-6 flex flex-col sm:flex-row justify-between rounded-lg border drop-shadow-xl bg-white p-6"
                  style={{
                ...
                >
                  ...
                  />
                  <div className="mt-5 sm:mt-0 sm:ml-4 sm:flex sm:w-full sm:justify-between">
                    <div>
                      <h2
                      ...
                      >
                        Quantity:
                        <button
                          onClick={() => handleQuantityUpdate(item, Math.max(1, quantity - 1))}
